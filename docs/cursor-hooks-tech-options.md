...
│                        Cursor IDE                                 │
│  ┌─────────────┐    stdin/stdout JSON    ┌─────────────────────┐ │
│  │ Agent 对话  │ ──────────────────────► │ .cursor/hooks/aix_ingest.py │
│  │ 生命周期    │                         │ 或 .sh / .py          │ │
│  └─────────────┘                         └──────────┬──────────┘ │
└─────────────────────────────────────────────────────┼────────────┘
                                                      │ HTTP POST
...
| `beforeSubmitPrompt` | 用户提交 prompt 前 | `POST /api/ingest/messages`（messageType=1 user） | **是** | 核心采集点 |
| `afterAgentResponse` | AI 完整回复后 | `POST /api/ingest/messages`（messageType=2 assistant） | **是** | 核心采集点 |
| `stop` | Agent 任务结束 | `POST /api/ingest/sessions/{id}/end` | **是** | 触发摘要 / 知识抽取（§8.2） |
| `afterAgentThought` | Agent 完成一段 thinking 块 | `POST /api/ingest/messages`（messageType=3 thought，event=18） | **是** | 同轮可有多段 thought，messageId 按正文 hash |
...
- **v1 默认提供 Python 3 脚本**（`aix_ingest.py`），单入口处理所有事件，通过 `hook_event_name` 或 payload 字段区分；
- **hooks.json 直接调用 Python**（`python -u .cursor/hooks/aix_ingest.py`），不再使用 PowerShell 薄封装（Windows 管道会破坏长 JSON）。
...
```json
{
  ...
  "hooks": {
    "sessionStart": [
      {
        "command": "python -u .cursor/hooks/aix_ingest.py",
        "timeout": 3
      }
    ],
    "beforeSubmitPrompt": [
...
| `afterAgentResponse` | `conversation_id`, `text`, `generation_id`, `transcript_path` | `sessionId`, `content`, `clientMessageId` |
