...
```
用户消息入库 → 清洗（去噪、截断）→ Spring AI EmbeddingModel → VectorStore(Milvus) → 更新索引状态
```
...
- 模块化：通过 **Spring AI** 抽象切换 Embedding 模型、VectorStore（Milvus）、ChatModel（LLM）；
...
```
┌─────────────────┐     MCP (stdio/SSE)      ┌──────────────────────────────────────┐
│  AI Client      │ ◄──────────────────────► │  AI-X（Spring Boot 3.x + Spring AI） │
│  Cursor/Claude  │                          │  ┌────────────────────────────────┐  │
└─────────────────┘                          │  │ ai-x-mcp-server / ai-x-api     │  │
                                             │  └───────────────┬────────────────┘  │
                                             │                  ▼                   │
                                             │  ┌────────────────────────────────┐  │
                                             │  │ ai-x-core（领域服务）            │  │
...
**存储分工：** 会话、消息、知识条目等结构化业务数据存 **MySQL**；语义向量通过 **Spring AI `VectorStore`（Milvus 实现）** 管理。业务表与向量索引职责分离，更利于 RAG 检索调优与水平扩展。

**AI 能力统一由 Spring AI 提供：** 向量化、向量检索、文档切分、RAG 编排、LLM 调用均通过 Spring AI 抽象接入，不引入 LangChain4j 等其他 AI 框架。

### 7.2 技术选型建议

| 层次 | 推荐方案 | 说明 |
|------|----------|------|
| 语言 | **Java 21** | LTS；虚拟线程、Record、Pattern Matching 等 |
| 基础框架 | Spring Boot 3.x | Web、调度、配置、依赖注入 |
| **AI / RAG 框架** | **Spring AI** | Embedding、VectorStore、ChatClient、Document、RAG Advisor |
| MCP SDK | 官方/社区 Java MCP SDK | 实现 Server 协议 |
| ORM | MyBatis-Plus | 实体映射；注意 `@TableField` 驼峰转下划线 |
| 关系库 | **MySQL 8.x** | Session / Message / Knowledge 等业务 ACID 存储 |
| 向量库 | **Milvus 2.x**（经 Spring AI VectorStore） | 稠密向量 ANN 检索 |
| 缓存/队列 | Redis + Spring `@Async` | embedding 异步、RAG 检索缓存 |
| 模型接入 | Spring AI Model Starters | OpenAI / Ollama / 兼容 OpenAI API 的本地模型 |
| 分析 | 内置算法 + Spring AI ChatClient | 薄弱区聚类、会话摘要、知识抽取 |
| 构建 | Maven / Gradle | 与团队习惯一致 |

### 7.2.1 Spring AI 组件与 RAG 链路

AI-X 的 RAG 链路：**索引（Ingest）→ 检索（Retrieve）→ 重排（Rerank，可选）→ 生成（Generate）**，全部由 Spring AI 编排。

| Spring AI 组件 | 在 AI-X 中的作用 | 典型 Starter / 实现 |
|----------------|------------------|---------------------|
| `EmbeddingModel` | 消息与知识条目向量化 | `spring-ai-openai` / `spring-ai-ollama` |
| `VectorStore` | Milvus 向量读写与 ANN 检索 | `spring-ai-milvus-store` |
| `Document` + `TextSplitter` | 长知识分块、`metadata` 携带 sourceRef | `TokenTextSplitter` |
| `VectorStoreDocumentRetriever` | `search_history` / `search_knowledge` 召回 | topK 可配置 |
| `RetrievalAugmentationAdvisor` | RAG 上下文注入与 Prompt 组装 | MCP 检索类 Tool |
| `ChatClient` | 会话摘要、知识抽取、RAG 回答合成 | 同 Embedding 提供商 |
| `@Observed` / Micrometer | retrieve / embed / chat 耗时指标 | 可观测性 |
...
```
用户 query
  → EmbeddingModel.embed(query)          // Spring AI
  → VectorStore.similaritySearch(topK=20) // Spring AI → Milvus
  → [可选] Reranker → topN=5
  → RetrievalAugmentationAdvisor 注入上下文
  → ChatClient 生成（或 MCP Tool 直接返回检索片段）
```
...
Message / KnowledgeEntry
  → 文本清洗（去噪、截断）
  → [Knowledge 长文] TextSplitter 分块    // Spring AI
  → EmbeddingModel.embed(chunks)
  → VectorStore.add(documents)            // Spring AI → Milvus
  → MySQL 更新 embedding_status
```

**v1.1 增强（仍基于 Spring AI 扩展，不引入其他框架）：**

| 能力 | 方案 |
|------|------|
| 重排序 | 自定义 `DocumentPostProcessor` 或接入 BGE-Reranker |
| 混合检索 | Milvus 向量 + MySQL FULLTEXT，结果在 Advisor 层融合 |
...
├── ai-x-core            # 领域模型、业务服务
├── ai-x-storage         # MyBatis-Plus、Repository（MySQL）
├── ai-x-rag             # Spring AI 统一模块：EmbeddingModel、VectorStore(Milvus)、ChatClient、RAG Pipeline
├── ai-x-analysis        # 薄弱区、聚类、报告（向量读取经 ai-x-rag）
...
sequenceDiagram
    ...
    participant RAG as Spring AI (ai-x-rag)
    participant Vec as VectorStore (Milvus)
    ...
    Svc-->>MCP: messageId
    MCP-->>Client: success
    Q->>RAG: embed & add to VectorStore (async)
    RAG->>Vec: VectorStore.add(documents)
```
...
sequenceDiagram
    ...
    participant RAG as Spring AI ChatClient
    participant Vec as VectorStore (Milvus)

    MCP->>Svc: end_session(sessionId)
    Svc->>RAG: extract knowledge candidates
    RAG-->>Svc: structured candidates
    Svc->>Svc: classify personal/common
    Svc->>Vec: dedupe by similaritySearch
...
| 配置键 | 说明 | 默认值 |
|--------|------|--------|
| spring.ai.model.embedding | Embedding 模型 | ollama:nomic-embed-text |
| spring.ai.model.chat | Chat 模型 | ollama:llama3 |
| spring.ai.vectorstore.milvus.host | Milvus 地址 | localhost:19530 |
| spring.ai.openai.api-key | OpenAI Key（可选） | — |
| spring.ai.ollama.base-url | Ollama 地址 | http://localhost:11434 |
| aix.rag.chunk-size | 知识分块 token 数（Spring AI TextSplitter） | 512 |
| aix.rag.chunk-overlap | 分块重叠 token 数 | 64 |
| aix.rag.retrieve-top-k | VectorStore 召回数 | 20 |
| aix.rag.rerank-top-n | 重排后保留数 | 5 |
| aix.rag.rerank.enabled | 是否启用重排 | false |
...
| **M2：向量检索** | Spring AI Embedding + VectorStore + search_history / search_knowledge | 2 周 |
...
**外部依赖：** Spring AI 模型提供商（OpenAI / Ollama 等）、MySQL 8.x、Milvus 2.x、Redis。
...
| RAG | Retrieval-Augmented Generation，检索增强生成：先召回相关片段再交由 LLM 合成回答 |
| Milvus | 开源向量数据库，用于稠密向量 ANN 检索 |
| Spring AI | Spring 官方 AI 框架；AI-X 全部 AI 能力（Embedding、VectorStore、ChatClient、RAG）均基于此实现，不引入其他 AI 框架 |
